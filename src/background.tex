...
Peira is a framework for systematic conduct of experimental studies for the purpose of evaluating the appropriateness of conceptual modeling language vocabularies \cite{LZMK.24}. \red{The method is based on having experimental participants classify samples of chunks of domain information to terms of a language, to which they have been previously trained and whose vocabulary is the subject of investigation vis-\`{a}-vis its quality. Peira offers a systematic approach to translate the resulting classification dataset into quality indicators.}

%\red{More formally,} an instance of a Peira experiment is meant to evaluate a set $V$ of \textit{signifiers} representing the vocabulary of the language to be evaluated. To do so, a sample of descriptions $E$ is taken from one or more application domains where the language is meant to be used. 

\red{More formally,} consider a set $V$ of \textit{signifiers} representing the vocabulary of a language to be evaluated. For the evaluation, a sample of descriptions $E$ is taken from one or more application domains where the language is meant to be used. Further, a set of $\mathbf{D} = D \cup R$ of distinguished elements $D$ from each domain, combined with a set of $R$ n-tuples thereof, are defined as a sample of the Universe of Discourse (UoD) relating to the domain and deemed as worthy of being modeled using the vocabulary. Domain elements from $\mathbf{D}$ under specific descriptions are referred to as \textit{subjects} from a set $S = \mathbf{D} \times E$. A set $P$ of \textit{raters} are also identified as a sample (or a proxy thereof) from a population of potential users of the language whose vocabulary is in question. The role of the raters is to study each description  $e\in E$ and classify each subject $s\in S$ associated with $e$ to none, one, or more of the terms in vocabulary $V$.

As an example, consider a simple domain-specific language for describing enrollment and attendance of students to courses and lectures, using concepts \textit{course}, \textit{student}, \textit{lecture}, (entities / unary concepts) \textit{enrolls}, \textit{includes lecture}, and \textit{attends lecture} (relationships / binary concepts). Our evaluation question is if the vocabulary $V = $ \{``course'', ``student'', ... \} --- the English words representing the above concepts --- is appropriate for describing situations involving students enrolling courses, attending lectures, etc. A set of descriptions $E$ is prepared of real or contrived cases of students taking courses, attending lectures etc. Consider that in one of them $e\in E$, Alice decided to take Statistics only to find that it is hard to follow and that she fell asleep during the lecture on the Central Limit Theorem (CLT). Within the context of the case, elements of  $D$ include ``Alice'', ``Statistics'', ``Lecture on CLT'' and elements of $R$ include tuples such as ``(Alice, Statistics)'', ``(Alice, Lecture on CTL)'', and ``(Statistics, Lecture on CTL)''. These elements combined constitute subjects $s\in S$ under the description of Alice's case $e$. 
\red{The evaluators include in $S$ only subjects that represent domain information which they believe should capturable by the language in question.} These subjects are to be classified under none, one, or more of the elements in $V$ by a sample of participants who are likely to use the vocabulary, such as the maintainers and analysts/instantiators of a software product line for course enrollment and attendance tracking.
...
%Specifically, for a specific term $v$, $\mathit{acc}(\cdot,\cdot,v)$ represents the number of ratings in which participants and designers are in agreement with respect to term $v$, $\mathit{def}(\cdot,\cdot,v)$ the number of authoritative ratings that were not picked by participants (``false negatives''), and $\mathit{exc}(\cdot,\cdot,v)$ the participant ratings that are in excess of the authoritative ones (``false positives''). 
It follows that, in the more familiar terms of precision and recall:
...
\red{Importantly, Peira accepts that quality indicators do not characterize the language absolutely, but relatively to a group of raters and the training the raters have received on the language. 
%In other words, the same language may perform differently if raters are sampled from different populations and are trained in different ways or to different levels.}
Hence, Peira evaluations aimed at assessing the quality of a language vis-\`{a}-vis a specific user population, require that raters are sampled from that population, or a population that can convincingly act as a proxy thereof.
}
%It follows that, ideally, raters are sampled from the prospective user population and are subjected to the level and kind of training that such users are expected to through before becoming productive with the language.}
%For example, a language can exhibit good quality when used by trained experts (e.g., its designers) but exhibit issues when used by persons with different backgrounds and with limited training on it. The significance of this depends on what the target user population is and }
