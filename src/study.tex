...
The Peira process is based on recruiting, training, and having experimental participants perform rating tasks. This entails cost and effort for those involved. With the advent of Large Language Models (LLMs) \cite{LLM.survey} the question of whether such models can act as more accessible and cost-effective substitutes for human input becomes relevant. We specifically propose that LLMs can act as substitutes for humans in Peira experiments only if their performance in Peira tasks is indistinguishable from that of humans. To appreciate the need for indistinguishability, recall that Peira's results are useful if the sample of raters is taken from the population of intended users of the language. It follows that, unless LLMs are the intended users of the language, their input is useful for drawing conclusions about a human group only insofar as it can be shown that LLM performance is representative of that of the human group. If that cannot be shown, substitutability is not warranted and basing conclusions on LLM data gives a false impression about language quality vis-\`{a}-vis the human group. If, for example, LLMs outperform humans, they may give to evaluators an impression of language quality that may not emerge among actual users. At the same time, as increased LLM size is known to improve capability but also increase cost and environmental impact, it is also useful to know how small an LLM can be and still play the role of a faithful substitute for human participants. The experimental study we report below addresses these research goals. The specific research questions (RQs) are:
...
\textbf{H$_0^2$.} Model size has no relationship with performance, measured as F1 score.




To investigate H$_0^1$, we form the corresponding confidence interval and check whether the precision and recall of each LLM fall within the interval. To evaluate H$_0^2$ we fit a simple linear regression model using log-transformed model size as predictor and the F1 score (harmonic mean of precision and recall) as response variable.
